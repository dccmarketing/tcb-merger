...
class edge_merger_Actions_and_Filters {
	...
	public function admin_scripts_and_styles( $hook ) {
		...
		wp_enqueue_style( 'edge-merger-admin', get_stylesheet_directory_uri() . '/admin.css' );
	...
	public function background_images() {
		...
		global $edge_merger_themekit;

		$output = '';
		$image 	= $edge_merger_themekit->get_thumbnail_url( get_the_ID(), 'full' );
	...
	/**
	 * Flush out the transients used in edge_merger_categorized_blog.
	 */
	function category_transient_flusher() {
		...
		// Like, beat it. Dig?
		delete_transient( 'edge_merger_categories' );
	...
	public function login_scripts() {
		...
		wp_enqueue_style( 'edge-merger-login', get_stylesheet_directory_uri() . '/login.css', 10, 2 );
	...
	public function public_scripts_and_styles() {
		...
		wp_enqueue_script( 'edge-merger-navigation', get_template_directory_uri() . '/js/navigation.min.js', array(), '20120206', true );

		wp_enqueue_script( 'edge-merger-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.min.js', array(), '20130115', true );

		wp_enqueue_style( 'dashicons' );

		wp_enqueue_script( 'edge-merger-search', get_template_directory_uri() . '/js/hidden-search.min.js', array(), '20150807', true );
